...
var path = require('path'),
    ...
    {isNormalTweet, getTweetText, isRetweet} = require('./tweets')
...
app.all(`/${process.env.BOT_ENDPOINT}`, async function(req, res){
  ...
  const USERNAME = "MichelAdamNew";
  const USERNAMES= ["MichelAdamNew", "prabowo", "rockygerung", "RelawanProSandi", "Fahrihamzah", "SaracenOfficial", "HukumDan", "FahryBakrie","loembok", "orangberiman6", "DahnilAnzar"]
...
function parseTweet(tweet) {
  let words = tweet
    .replace(/\&amp\;/gi, "&")
    .split(" ")
    .filter(word => !word.startsWith("@") || word.toLowerCase() === '@prabowo' || word.toLowerCase() === '@sandiuno')
    .filter(
      word =>
        !word.match(
