...
class Markov {
   ...
   * @memberof Markov
   */
  buildCorpus(data = this.data) {
    console.log("data length", data.length);
    if (data.length < 1) {
      return;
    }
    if (this.isBuilding) {
      console.log("Still building corpus, queueing data");
      this.queuedData.push(data);
      return;
    }
    console.log("Building corpus");
    this.isBuilding = true;
    const strings = data.map(s => (lodash_1.isString(s) ? { string: s } : s));
    ...
    strings.forEach((item, index) => {
      ...
      // End words
      const end = lodash_1
        .slice(words, words.length - stateSize, words.length)
        .join(" ");
      ...
      for (let i = 0; i < words.length - 1; i++) {
        ...
        const next = lodash_1
          .slice(words, i + stateSize, i + stateSize * 2)
          .join(" ");
